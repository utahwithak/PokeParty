...
- [ ] M8 — Optimal-play battle search (shield & switch decision search / minimax)
  ...
  - [x] M8.3 — 3v3 optimal shields + information-aware switching. Leads UI-designated (position 0, `LEAD` badge on both teams) — NO lead enumeration.
    ...
    - [x] Catch & sac swaps (2026-07-23). **Catch:** when the revealed opponent's banked energy (revealed info — players count fast moves) affords a charged move that's super-effective vs the active, and the switch timer allows, swap into a backup that resists it, survives the throw, and beats `catchMinRating`. **Sac:** out of shields, with the active winning but a banked move threatening ≥ `sacDangerFraction` of its remaining HP, throw a ≤ `sacMaxHpFraction`-HP backup in front as one more shield (the faint replacement is free). Both ride the existing interrupt machinery via a new opponent-energy trigger (armed only while false, so a declined offer can't re-interrupt until the energy dips). Boundary priority: escape → catch → sac (`boundarySwitchTarget`). Tested (organic catch run + decision-level sac).
